...
def feature_from_row(Community, commId, Address, Mission, MissionType, City, CommunityType, longitude,latitude, Website,Cec_status,legislative_district):
    ...
    'properties': {'CommunityPartner': '', 'Address': '', 'Projects':'',
               ...
               'communityprojects':[{'name':'','academicYear':[],'campuspartner':[],'engagementType':'','collegeNames':[]}]
               }
    ...
    if len(projectsCount) > 0:
        ...
        for x in projectsCount:
            ...
            projObj = {'name':str(projname)+';'+ str(start_academic_year), 'academicYear':projYearList,'campuspartner' : campList,'engagementType':eng_type,'collegeNames':collegeList}
...
if commPartnerList is not None:
    ...
    for obj in commPartnerList:
        ...
        print('Community, commId, Address, Mission, MissionType, City, CommunityType, longitude,latitude, Website,Cec_status,legislative_district')
    
        print('-feature row-',obj[0],obj[1],fulladdress,obj[7],obj[8],obj[4],obj[11],obj[13],obj[14],obj[12],obj[15],obj[9])
        feature = feature_from_row(obj[0],obj[1],str(fulladdress),obj[7],obj[8],obj[4],obj[11],obj[13],obj[14],obj[12],obj[15],obj[9])
    ...
    print("Partner GEOJSON file written having total records of " +repr(len(commPartnerList))+" in S3 bucket "+settings.AWS_STORAGE_BUCKET_NAME +" at " +str(currentDT))
    logger.info("Partner GEOJSON file written having total records of " +repr(len(commPartnerList))+" in S3 bucket "+settings.AWS_STORAGE_BUCKET_NAME +" at " +str(currentDT))
else:
     print("Partner GEOJSON file NOT written having total records of " +repr(len(commPartnerList))+" in S3 bucket "+settings.AWS_STORAGE_BUCKET_NAME +" at " +str(currentDT))
     logger.info("Partner GEOJSON file NOT written having total records of " +repr(len(commPartnerList))+" in S3 bucket "+settings.AWS_STORAGE_BUCKET_NAME +" at " +str(currentDT))
