...
with eng_type_filter as (select e.name engagement_type
      ...
      , count(distinct p.project_name) Projects
      , array_agg(distinct p.project_name) projects_id
...
def createproj_othermission(subcategory):
    return (
                """select secondary_mission_area_id from projects_missionsubcategory pms inner join projects_subcategory ps on ps.id = pms.sub_category_id where ps.sub_category ='""" + subcategory + """';""")


def createproj_addothermission(subcategory, projid):
    return (
                """insert into projects_projectmission (mission_type,mission_id,project_name_id) values ('Other','""" + subcategory + """','""" + projid + """'); """)
