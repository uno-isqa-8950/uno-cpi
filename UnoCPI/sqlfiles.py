...
from ((with focus_filter as (select pm.mission_id focus_id
                             ...
                             from home_missionarea m
                                 left join projects_projectmission pm on m.id = pm.mission_id and pm.mission_type = 'Primary'
       ...
       order by focus_name)) focus_data
order by rec_type, focus_id;
'''
...
from ((with focus_filter as (select pm.mission_id focus_id
      ...
      (with topic_filter as (select ms.secondary_mission_area_id focus_id
                                  ...
                                  , sum(p.total_k12_students) numberofk12students
                                  , sum(p.total_k12_hours) k12studentshours			   
                             from projects_subcategory subc
                                 join projects_missionsubcategory ms on ms.sub_category_id = subc.id
       ...
       from projects_subcategory topic
           join projects_missionsubcategory focus_topic on focus_topic.sub_category_id = topic.id 
