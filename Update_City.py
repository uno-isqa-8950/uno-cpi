...
output_filename = os.path.join(dirname,'home/static/GEOJSON/City.geojson') #The file will be saved under static/GEOJSON
...
if (conn):
    print("connection sucess in Update City--",conn)
    logger.info("Connection Successful!")
else:
    logger.info("Connection Error!")

logger.info("Get all the City Council Districts from the Database")
...
print(repr(len(dfCommunity)) +" City Council Partners fetched from the Database on " + str(currentDT))

if len(dfCommunity) == 0:
    logger.critical("No City Council Partners fetched from the Database while updating lang and lng on " + str(currentDT))
else:
    cursor = conn.cursor()
    #print('project name',dfCommunity['Community_Partner'])
    logger.info(repr(len(dfCommunity)) + "City Council Partners are in the Database while updating lang and lng on " + str(currentDT))
    ...
    if (geocode_result and geocode_result[0]):
        ...
        for i in range(len(district)):  # iterate through a list of district polygons
            ...
            if polygon.contains(coord):  # check if a partner is in a polygon
                legi_district = property["properties"]["OBJECTID"]
...
if len(dfCommunity) != 0:
    logger.info("Call update City Council Partners in database")
...
logger.info("City Council Partners of  " + repr(len(dfCommunity)) + " records are generated at " + str(currentDT))
