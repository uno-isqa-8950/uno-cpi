...
describe('Charts Focus Area Analysis Campus User', () => {
  ...
  it('Reset Filters', function() {
    ...
    cy.get('#select2-id_academicyear-container > .select2-selection__placeholder').contains('2018-19')
    ...
    cy.get('#select2-id_endacademicyear-container > .select2-selection__placeholder').contains('2019-20')
  ...
  //Filter options
  it('Analytic Charts Focus Area Analysis Campus User', function() {
    cy.get("[data-cy='analytics']").click().should('be.visible')
    cy.get("[data-cy='charts']").next("[data-cy='chartsdropdown']").then($el => {
      cy.wrap($el).invoke('show').should('be.visible')
      cy.wrap($el).get("[data-cy='Focus Area Analysis']").click()
