...
describe('Analytic Reports Campus Partner User', () => {
   ...
   it('Check navigation', function() {
    ...
    it.only('Hide Filters', function() {
        const hidefilterButton = '[data-cy=hidefilterbtn]'
        cy.get('#analyticnav').click()
        cy.contains('Reports').next('.dropdown-menu').then($el => {
            cy.wrap($el).invoke('show')
            cy.wrap($el).contains('Engagement Type').click()
        })
        cy.get(hidefilterButton).should('have.class', 'btn btn-secondary')
        cy.get(hidefilterButton).click()
    ...
    it.only('Reset Filters', function() {
        ...
        cy.get(resetFilterButton).click()
        cy.get('#select2-id_academic_year-container > .select2-selection__placeholder').contains('Previous Academic Year')
        })
    // Filter Options    
