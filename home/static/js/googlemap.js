...
var CollegeNamelist = JSON.parse(document.getElementById('college-list').textContent);
...
var select = '';
select += '<a href="#" ' + 'id=' + '"alltypes" ' + 'value=' + '"alltypes"><span style="background-color: #ffffff; border: 1px solid #ffffff"></span><b>All Community Partner Type</b></a>' + "<br>";
for (var i = 0; i < CommunityType.length; i++) {
    var color = colorcode[i]
    var communityType = CommunityType[i]
    select += `<a href="#"  id="${communityType.valueOf()}" value="${communityType.valueOf()}"><span style="background-color: ${color}"></span><b>${communityType.toString()}</b></a>` + "<br>";
}
$('#legend').html(select);
//*********************************** Add the districts *****************************************************
...
var select2 = '';
select2 += '<option val=' + "allmiss" + ' selected="selected">' + 'All Mission Areas' + '</option>';
...
// circle added to the map
    ...
    for (i=0; i<communityData.features.length; i++) {
        ...
        var communityType = communityData.features[i].properties["CommunityType"]
